...
public class WrapperMethods {
	...
	public boolean isElementDisplayedandGetText(WebDriver driver, String xpath, String Element_Name, int t) {
		...
		try {
			...
			Extent_Reporting.Log_Pass(Element_Name + "h is not displayed ", Element_Name + " is not displayed ");
			flag = false;
		}
		return flag;
	...
	public void enterText(AndroidDriver<MobileElement> screenName, String ObjectxPath, String sValue, String Element_Name)
			throws Exception {
		try {
			System.out.println("entering in enter text wrapper method: "+ObjectxPath);
			MobileElement inputText = screenName.findElement(By.id(ObjectxPath));
			//System.out.println("warasper  mehtiht");
			inputText.sendKeys(sValue);
		//	Extent_Reporting.Log_Pass(Element_Name + " Entered", sValue + " entered in " + Element_Name);
		} catch (Throwable t) {
		//	Extent_Reporting.Log_Fail(Element_Name + " does not Exist", Element_Name + " does not Exist", screenName);
	...
	public void waitForElementVisible(AndroidDriver<MobileElement> driver, String element) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, 30);
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(element)));
		} catch (NoSuchWindowException e) {
	...
	public void waitForElementClickable(AndroidDriver<MobileElement> driver, String element, String text) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, 30);
	...
	public void waitForElementClickable_ById(AndroidDriver<MobileElement> driver, String element, String text) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, 30);
			wait.until(ExpectedConditions.elementToBeClickable(By.id(element)));
		} catch (NoSuchWindowException e) {
	 ...
	 * driver.manage().timeouts().implicitlyWait(Integer.parseInt(new
	 * Common_Functions().GetXMLTagValue(Constants.configPath+"Config.xml",
	...
	public List<MobileElement> ClickbtnByClass(AndroidDriver<MobileElement> screenName, String ObjectxPath, String Element_Name) throws Exception {
		Thread.sleep(500);
		try {
			WebElement webButton = screenName.findElement(By.className(ObjectxPath));
	...
	public void ClickbtnById(AndroidDriver<MobileElement> screenName, String ObjectxPath, String Element_Name) throws Exception {
		Thread.sleep(500);
		try {
			WebElement webButton = screenName.findElement(By.id(ObjectxPath));
	...
	public void ClickbtnByXpath(AndroidDriver<MobileElement> screenName, String ObjectxPath, String Element_Name) throws Exception {
		Thread.sleep(1000);
		try {
			WebElement webButton = screenName.findElement(By.xpath(ObjectxPath));
	...
	public void selectDate(AppiumDriver<MobileElement> driver, String requiredDate, String Element_Name) throws InterruptedException {
try{
		String getDay = requiredDate.split("-")[0];
		String getMonth = requiredDate.split("-")[1];
		String getYear = requiredDate.split("-")[2];
		...
		//Extent_Reporting.Log_Pass(requiredDate, "Date selected");
}catch(Exception e){
	Extent_Reporting.Log_Fail("fail", "Date not selected", driver);
	Report_Setup.extent.flush();
	e.printStackTrace();
	}
}

	public void verticalSwipeByPercentages(AppiumDriver<MobileElement> driver, double startPercentage,
		...
		new TouchAction(driver).press(PointOption.point(x, startPoint))
			.waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))).moveTo(PointOption.point(x, endPoint))
				.release().perform();
	}
	
	public void horizontalSwipeByPercentage (AppiumDriver<MobileElement> driver,double startPercentage, double endPercentage) {
        Dimension size = driver.manage().window().getSize();
        ...
        new TouchAction(driver)
                .press(PointOption.point(startPoint, y))
	...
	public void selectNumber(AppiumDriver<MobileElement> driver, String requiredNo) throws InterruptedException {

		String getNo = "5";
		while (true) {
			verticalSwipeByPercentages(driver, 0.8, 0.2);

			if (driver.findElement(By.id("android:id/numberpicker_input")).getText().equals(getNo)) {
				break;
		...
		driver.findElement(By.id("com.tourisz:id/btnSet")).click();
